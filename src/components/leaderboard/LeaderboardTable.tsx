...
export function LeaderboardTable({ entries, currentUserId, title = "Leaderboard", loading }: Props) {
  if (loading) {
    return (
      <section className="mt-4 rounded-2xl border border-border/70 bg-card/70 p-4 shadow-lg shadow-black/10 backdrop-blur max-sm:mt-4 sm:mt-6">
  ...
  return (
    <section className="mt-4 max-sm:mt-4 sm:mt-6">
      ...
      {entries.length === 0 ? (
        <p className="rounded-2xl border border-border/70 bg-card/70 p-4 text-muted-foreground shadow-lg shadow-black/10 backdrop-blur max-sm:text-xs sm:text-sm">No scores yet. Make predictions and wait for results to be settled.</p>
      ) : (
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card/80 shadow-xl shadow-black/15 ring-1 ring-primary/5 backdrop-blur max-sm:rounded-xl">
